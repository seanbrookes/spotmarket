Org.service('OrgServices', [
  ...
  function(Org, smGlobalValues, OrgSessionService, UserLocation, ProfileAvatar, AliasEmail, $log) {
    ...
    svc.updateCurrentOrgPosition = function(position) {
      if (position && position.geometry) {
        var userId = OrgSessionService.getValueByKey('smHandle');
        OrgSessionService.getOrgProfileById(userId)
          .then(function(responseOrg) {
            responseOrg.lastPosition = position;
            svc.saveOrg(responseOrg)
              .then(function(responseUpdatedOrg) {
                OrgSessionService.setValueByKey('smCurrentPosition', JSON.stringify(position));
                return responseUpdatedOrg;
              })
    ...
    svc.saveOrg = function(org) {
      if (!org.createdDate) {
        org.createdDate = (new Date).getTime();
      }
      org.lastUpdate = (new Date).getTime();
      if (org.id) {
        return Org.upsert(org)
          .$promise
          .then(function(response) {
            return response;
          });
    ...
    svc.deleteOrg = function(userId) {
      if (userId) {
        //return Org.deleteById({id:userId})
        //  .$promise
        //  .then(function(response) {
        //    return response;
        //  });
    ...
    svc.findOrgByEmail = function(email) {
      if (email && svc.isValidEmail(email)) {
        return Org.find({filter:{where:{authEmail:email}}})
          .$promise
          .then(function(response) {
            return response;
          })
    ...
    svc.getOrgs = function(filter) {
      if (!filter) {
        filter = {};
      }
      return Org.find(filter)
        .$promise
        .then(function(response) {
          return response || [];
        });
    };
    svc.createInitialOrgProfile = function() {
      return Org.create({createdDate:new Date().getTime(), lastUpdate:new Date().getTime()})
        .$promise
        .then(function(currentOrgResponse) {
          if (currentOrgResponse.token) {

            OrgSessionService.setValueByKey('smToken', currentOrgResponse.token);

            return OrgSessionService.getCurrentOrgFromClientState();
          }
        })
        .catch(function(error) {
...
Org.service('OrgSessionService', [
  ...
  'UserLocation',
  'smGlobalValues',
  function(Org, $cookies, $log, $timeout, $http, UserLocation, smGlobalValues) {
    ...
    svc.setValueByKey = function(key, value) {
      ...
      smGlobalValues.currentOrg[key] = value;
      // synch user
      // - db
      // get user from
      // - local storage
    ...
    var arrayOfWordsOne = [];
    var arrayOfWordsTwo = [];
    svc.generateNewOrgTag = function(options) {
    ...
    svc.isCurrentOrgLoggedIn = function() {
    ...
    svc.registerExistingOrg = function(orgCtx) {
      return Org.registerExistingOrg(orgCtx)
        .$promise
    ...
    svc.clearCurrentCacheOrg = function() {
        // save the token
    ...
    svc.getOrgHandleSuggestionHistory = function() {
      var retVal = [];

      var rawString = window.localStorage.getItem('smOrgHandleHistory');
      if (rawString) {
    ...
    svc.addOrgHandleSuggestionToHistory = function(handleSuggestion) {
      if (handleSuggestion) {
        var rawArray = [];
        var rawString = window.localStorage.getItem('smOrgHandleHistory');
        if (!rawString) {
          window.localStorage.setItem('smOrgHandleHistory', JSON.stringify([handleSuggestion]));
        }
        else {
          var rawArray = JSON.parse(rawString);
          ...
          window.localStorage.setItem('smOrgHandleHistory', JSON.stringify(rawArray));
        }
      }

      return handleSuggestion;
    };
    svc.getCurrentOrgFromClientState = function() {
      var user = {};
      user.smHandle = svc.getValueByKey('smHandle');
      user.smSessionId = svc.getValueByKey('smSessionId');
      user.smOrgId = svc.getValueByKey('smOrgId');
      user.smOrgPreferences = svc.getValueByKey('smOrgPreferences');
      user.smToken = svc.getValueByKey('smToken');
      user.smEmail = svc.getValueByKey('smEmail');
      user.smOrgName = svc.getValueByKey('smOrgName');
      ...
      user.isCurrentOrgLoggedIn = svc.getValueByKey('isCurrentOrgLoggedIn');

      return user;

    };
    svc.getOrgProfileById = function(id) {
      return Org.find({userId: id})
        .$promise
        .then(function(response) {
          if (response && response.length && response.length > 0) {

            var returnOrg = response[0];
            var clientStateOrg = svc.getCurrentOrgFromClientState();
            return angular.extend(returnOrg, clientStateOrg);
          }
          else {
            return null;
          }
